...
int main(int argc, char **argv) {
  ...
  try {
    ...
    TCLAP::UnlabeledMultiArg<int> landmarksToModelArg(
          "landmarksToModel",
          "landmarks to train (if flag not present --> use all landmarks)",
          false, "landmarks-to-model");
    cmd.add(landmarksToModelArg);
    cmd.parse(argc, argv);

    string configPath = configFileArg.getValue();
    vector<int> landmarksToModel = landmarksToModelArg.getValue();

    // parse config file
    boost::property_tree::ptree configuration;
    boost::property_tree::ini_parser::read_ini(configPath, configuration);

    // creating logger
    log4cxx::LoggerPtr logger(log4cxx::Logger::getLogger("CPT.mainTraining"));
    ...
    // creating trainer
    faceAnalysis::TrainerPtr trainer =
        faceAnalysis::TrainerPtr(new faceAnalysis::Trainer(
                                   configuration, landmarksToModel));

    // train the local model
    if (configuration.get<bool>("training.trainLocal")) {
        trainer->TrainLocalWrapper();
    }

    // train the global model
    if (configuration.get<bool>("training.trainGlobal"))
      trainer->TrainGlobal();
